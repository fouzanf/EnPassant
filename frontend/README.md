...
* **⚡ Algorithmic Exact Caching:** Bypasses external network latencies and token usage costs via an $O(1)$ database hash check. Repeat queries are caught, logged as a `[CACHE HIT]`, and served instantly in milliseconds.
* **🛡️ Sliding Window Rate Limiting:** Protects the endpoint from DDoS attacks and API spam by strictly limiting incoming requests per IP address using a custom in-memory sliding window algorithm (5 requests / 60 seconds).
* **🔑 Multi-Tenant Key Forwarding:** Enables external applications to connect to the shared proxy using their own AI billing accounts by passing their keys dynamically through the HTTP `Authorization` header.
* **📊 Live NOC Telemetry Dashboard:** A premium Next.js dashboard that polls system metrics dynamically to visualize total data throughput, cache efficiency, and specific intercepted PII counts.
...
```text
[ Next.js Frontend UI ]
         │ (Telemetry Polling & Interactive Playground requests)
         ▼
